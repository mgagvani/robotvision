...
class WaymoE2E(Dataset): 
    def __init__(self, indexFile = 'index.pkl', data_dir='./dataset'):
        ...
        self.file = open(os.path.join(self.DATA_DIR, self.indexes[0][0]), 'rb')
        self.filename=self.indexes[0][0]
    ...
    def __getitem__(self, idx):
        ...
        if self.filename != self.indexes[idx][0]:
            self.file.close()
            self.file = open(os.path.join(self.DATA_DIR, self.indexes[idx][0]), 'rb')
            self.filename = self.indexes[idx][0]

            # pass
        self.file.seek(self.indexes[idx][1])
        protobuff = self.file.read(self.indexes[idx][2])
