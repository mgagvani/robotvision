    ...
    with torch.no_grad():
        for batch in loader:
            batch_targets = extract_batch_targets(
                model, sae, capture, batch, device, latent_source
            )
...
def main() -> None:
    ...
    parser.add_argument("--latent_source", type=str, default=None, choices=LATENT_SOURCES)
    ...
    latent_source = (
        args.latent_source or checkpoint_value(residual_checkpoint, "latent_source") or "sae"
    )
    ...
    model, sae, capture, _ = load_model_and_sae(
    ...
    print(f"Using latent source: {latent_source}")
