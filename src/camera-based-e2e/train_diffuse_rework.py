...
from models.base_model import LitModel, collate_with_images_tokens_depth
from models.monocular import MonocularModel, DeepMonocularModel, SAMFeatures
from models.blocks import TransformerBlock
from utils import SinusoidalPosEmb, get_sinusoidal_embeddings
...
class DiffuseLitModel(pl.LightningModule):
    ...
    def _shared_step(self, batch, stage):
        ...
        if stage == "train":
            ...
            pred_x0, pred_depths = self.model(model_inputs, timesteps, stage)
            pred_x0 = pred_x0 * self.future_scale

            # print(depths.shape)
            depths = depths.permute(1, 0, 2, 3, 4) # (3, 12, 1, 128, 128)
            ...
            loss = recon_loss + depth_loss * 0.8
    ...
    def forward(self, x, t, stage):
        ...
        if stage == "train":
            ...
            for block in self.decoder_blocks[:4]:
                queries = block(queries, context[:, :tokens.size(1), :])
            
            for block in self.decoder_blocks[4:6]:
                queries = block(queries, context)
            
            for block in self.decoder_blocks[6:]:
                queries = block(queries, queries)

            waypoints = self.predict_waypoints(queries.squeeze(1))
            waypoints = waypoints.view(-1, 20, 2)
            
            return waypoints, depth_maps
            ...
            for t_step in self.scheduler.timesteps:
                ...
                pred_original_sample = controls.view(context.size(0), 20, 2)
...
if __name__ == "__main__":
    ...
    # Data 
    train_dataset = WaymoE2E(indexFile='index_train.pkl', data_dir=args.data_dir, images=True, n_items=330_000, use_precomputed=True, precomputed_dir=pre_dir+'train/')
    test_dataset = WaymoE2E(indexFile='index_val.pkl', data_dir=args.data_dir, images=True, n_items=25_000, use_precomputed=True, precomputed_dir=pre_dir+'val/')
