...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='waymo', choices=['waymo', 'nuscenes'], help='Which dataset to train on')
    ...
    if args.dataset == 'waymo':
        ...
        train_dataset = NuScenesDataset(data_dir=args.data_dir, split='train', n_items=250_000)
        test_dataset = NuScenesDataset(data_dir=args.data_dir, split='val', n_items=25_000)
        nw = 16
    else:
        raise ValueError(f"Unsupported dataset: {args.dataset}")

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, num_workers=nw, collate_fn=collate_with_images, persistent_workers=False, pin_memory=False)
    val_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, num_workers=nw, collate_fn=collate_with_images, persistent_workers=False, pin_memory=False)
