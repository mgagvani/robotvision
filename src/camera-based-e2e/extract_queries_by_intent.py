...
with torch.no_grad():
    for i, batch in enumerate(test_loader):
        ...
        match intent:
            case 0:
                straight.append(preds["query"].detach().cpu())
            case 1:
                left.append(preds["query"].detach().cpu())
            case 2:
                right.append(preds["query"].detach().cpu())

torch.save(straight, "straight_queries.pt")
torch.save(left, "left_queries.pt")
torch.save(right, "right_queries.pt")