...
class DiffuseLitModel(pl.LightningModule):
    ...
    # if random.random() > 0.5 else self.anchors.to(future.device)[random.choices(list(range(len(self.anchors))), k=len(future))[0]]
    ...
    def _shared_step(self, batch, stage):
        ...
        if stage == "train":
            ...
            future_noisy = self.scheduler.add_noise(future_norm, noisy_anchor, timesteps)
            ...
            noisy_anchors = anchors.to(past.device, dtype=past.dtype) +  noise
