...
class DiffuseLitModel(pl.LightningModule):
    ...
    def _shared_step(self, batch, stage):
        ...
        if stage == "train":
            ...
            pred_x0 = self.model(model_inputs, timesteps, stage) * self.future_scale
            ...
            model_inputs = {'PAST': past, 'IMAGES': images, 'INTENT': intent, 'FUTURE': noise}
...
class DiffusionLTFMonocularModel(nn.Module):
    ...
    def forward(self, x, t, stage):
        ...
        for cam in cams:
            ...
            # [16, 192, 1024]
            # tokens = tokens.permute(0, 2, 1) # [16, 192, 1024]
            tokens = tokens.view(tokens.size(0), tokens.size(1), 32, 32) # [16, 192, 32, 32]
