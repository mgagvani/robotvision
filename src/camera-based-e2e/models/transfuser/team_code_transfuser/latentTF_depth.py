...
def res_transform(front: torch.Tensor,
    ...
    def _to_bchw(x):
        ...
        # Keep image tensors in uint8-like scale [0, 255] as float.
        # Downstream normalize_imagenet() expects this scale and will divide by 255 internally.
        x = x.float()
        if x.numel() > 0 and x.max() <= 1.5:
            # If images were already normalized to [0, 1], undo that here.
            x = x * 255.0
        return x

    front = _to_bchw(front)
    front_left = _to_bchw(front_left)
    front_right = _to_bchw(front_right)

    # Resizing
    target_h, target_w_per = 160, 320
    front = TF.resize(front, [target_h, target_w_per], interpolation=InterpolationMode.BILINEAR, antialias=True)
    front_left = TF.resize(front_left, [target_h, target_w_per], interpolation=InterpolationMode.BILINEAR, antialias=True)
    front_right = TF.resize(front_right, [target_h, target_w_per], interpolation=InterpolationMode.BILINEAR, antialias=True)

    # Concat
    cont = torch.cat([front_left, front, front_right], dim=3)
    final = TF.resize(cont, [160, 704], interpolation=InterpolationMode.BILINEAR, antialias=True)
    return final # [B,3,160,704]

def input_transform(front_left, front, front_right, ego_speed, config, device):
    # 1. RGB
    rgb = res_transform(front, front_left, front_right)
    rgb = rgb.to(device=device, dtype=torch.float32)

    B = rgb.shape[0]
    H_l = config.lidar_resolution_height
    W_l = config.lidar_resolution_width
    lidar = torch.zeros((B, 2, H_l, W_l), device=device, dtype=torch.float32)

    # 3. velocity [B,1]
    if ego_speed is None:
        velocity = torch.zeros((B, 1), device=device, dtype=torch.float32)
    else:
        velocity = ego_speed
        if velocity.dim() == 1:
            velocity = velocity.unsqueeze(1)
        velocity = velocity.to(device=device, dtype=torch.float32)

    return rgb, lidar, velocity

class LatentTFModel(pl.LightningModule):
    def __init__(
        self,
        config,
        lr: float,
        ...
        # Backbone
        self.backbone = latentTFBackbone(
            config=config,
        ...
        # Prediction head: fused_features (512) -> future waypoints (20, 2)
        self.head = nn.Sequential(
            nn.Linear(config.perception_output_features, 256),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(128, 20 * 2)  # 20 waypoints, each (x, y)
        )
        
        self.use_depth_loss = use_depth_loss
        if self.use_depth_loss:
            # Depth head predicts a single-channel depth map from image_features_grid
            # Assuming image_features_grid is (B, 512, H, W), we'll predict (B, 1, 128, 128)
            self.depth_head = nn.Sequential(
                nn.Conv2d(config.perception_output_features, 256, kernel_size=3, padding=1),
                nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x: batch dict with keys:
            - 'IMAGES': list of camera tensors [cam0, cam1_FRONT_LEFT, cam2_FRONT, cam3_FRONT_RIGHT, ...]
            - 'PAST': (B, 16, 6) with columns [pos_x, pos_y, vel_x, vel_y, accel_x, accel_y]
        
        Returns:
            If use_depth_loss: dict with 'trajectory' and 'depth'
            Otherwise: trajectory tensor (B, 20, 2)
        """
        # Extract images from list
        # Waymo camera indices: 1=FRONT_LEFT, 2=FRONT, 3=FRONT_RIGHT
        images = x['IMAGES']
        front_left = images[1]
        front = images[2]
        front_right = images[3]
        
        # Extract ego speed from PAST velocities (vel_x, vel_y at timestep -1)
        past = x['PAST']  # (B, 16, 6)
        vel_x = past[:, -1, 2]  # Last timestep, vel_x
        vel_y = past[:, -1, 3]  # Last timestep, vel_y
        ego_speed = torch.sqrt(vel_x**2 + vel_y**2)  # Compute speed magnitude
        
        # Transform inputs
        rgb, lidar, velocity = input_transform(
        ...
        features, image_features_grid, fused_features = self.backbone(rgb, lidar, velocity)
        
        # Predict waypoints
        trajectory = self.head(fused_features)  # (B, 40)
        trajectory = trajectory.view(-1, 20, 2)  # (B, 20, 2)
        
        # Predict depth if enabled
        if self.use_depth_loss:
            pred_depth = self.depth_head(image_features_grid)  # (B, 1, 128, 128)
            pred_depth = pred_depth.squeeze(1)  # (B, 128, 128)
            return {'trajectory': trajectory, 'depth': pred_depth}
        else:
            return trajectory
    ...
    def _shared_step(self, batch, stage: str):
        if batch["FUTURE"].shape[1] == 0:
            return None
        
        pred = self(batch)
        
        # Handle dict output (with depth) or tensor output (without depth)
        pred_depth = None
        ...
        # Trajectory loss (ADE)
        loss_ade = self.per_example_ade(pred_trajectory, batch["FUTURE"]).mean()
        
        # Depth loss
        loss_depth = torch.tensor(0.0, device=self.device)
        if pred_depth is not None and self.use_depth_loss:
            front_img = batch["IMAGES"][2]  # front camera (index 2)
            # Downsample to 128x128 for depth prediction
            ...
            loss_depth *= self.hparams.depth_loss_weight
        
        # Total loss
        total_loss = loss_ade + loss_depth
        
        # Logging
        self.log(f"{stage}_loss_ade", loss_ade, prog_bar=True, logger=True)
        self.log(f"{stage}_loss_depth", loss_depth, prog_bar=True, logger=True)
        self.log(f"{stage}_loss", total_loss, prog_bar=True, logger=True)
...
class LidarEncoder(nn.Module):
    ...
    def __init__(self, architecture, in_channels=2):
        ...
        # Save bias before deleting _tmp if needed
        saved_bias = _tmp.bias if use_bias else None
        ...
        del _tmp
        # Only delete stem for architectures that have it (RegNet, ConvNext)
        if hasattr(self._model, 'stem'):
            del self._model.stem
        torch.cuda.empty_cache()
        
        if use_bias:
            self._model.conv1.bias = saved_bias
