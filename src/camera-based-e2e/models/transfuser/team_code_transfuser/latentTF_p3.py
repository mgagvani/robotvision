...
def res_transform(front: torch.Tensor,
    ...
    return: [B, 3, 160, 704]
    Since our resolution ratio is 1.5, we will need to crop to match the ratio and then resize.
    """

    # consistent tensor format [0,1]
    def _to_bchw(x):
        if x.dim() == 3:
            x = x.unsqueeze(0)
    ...
    # Resizing
    def crop_then_resize(img):
        B, C, H, W = img.shape
        target_h = W // 2
        if target_h > H:
            target_h = H
        top = (H - target_h)
        img = TF.crop(img, top=top, left = 10, height=target_h, width=W) # crop to 960x1920
        img = TF.resize(img, [160, 320], interpolation=InterpolationMode.BILINEAR, antialias=True) # resize to 160x320
        return img

    front = crop_then_resize(front)
    front_left = crop_then_resize(front_left)
    front_right = crop_then_resize(front_right)

    # Concat
    cont = torch.cat([front_left, front, front_right], dim=3) # [B,3,160,960]

    left = (960 - 704) // 2    
    final = TF.crop(cont, top=0, left=left, height=160, width=704) # crop to 160x704
