...
class DeepMonocularModel(nn.Module):
    ...
    def forward(self, x):
        ...
        occ_logits = self.occ_head(feats)  # (B, 96, h, w)
        occ_logits = F.interpolate(occ_logits, size=(100, 100), mode="bilinear", align_corners=False)

        B = occ_logits.size(0)
        occ_logits = occ_logits.view(B, 6, 16, 100, 100)
        occ_logits = occ_logits.permute(0, 1, 3, 4, 2).contiguous()  # (B, 6, 100, 100, 16)
        # tokens: handle list of features or single tensor
