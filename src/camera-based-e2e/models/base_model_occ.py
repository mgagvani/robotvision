...
class LitModel(pl.LightningModule):
    ...
    def _shared_step(self, batch: torch.Tensor, stage: str) -> torch.Tensor:
        ...
        if isinstance(pred_future, dict):
            pred_depth = pred_future.get("depth", None)
            pred_scores = pred_future.get("scores", None)
            pred_occ = pred_future.get("occ", None)
            pred_future = pred_future["trajectory"]
        ...
        if pred_occ is not None and "OCC" in batch:
            occ_gt = batch["OCC"].long()  # (B, 100, 100, 16)
            loss_occ = F.cross_entropy(pred_occ, occ_gt, ignore_index=255)
        else:
        ...
        loss_score *= 1.0
        total_loss = loss_ade + loss_depth + loss_score + loss_rfs + loss_occ
...
def collate_with_images(batch):
    ...
    if batch[0].get("OCC", None) is not None:
        out["OCC"] = torch.stack(
