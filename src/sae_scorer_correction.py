...
def latent_features(
    ...
    """Features the residual adapter consumes.

    "sae" encodes the captured activation into the sparse latent space; "raw"
    is the paper's control arm, feeding the same adapter the dense activation
    the SAE was trained to reconstruct.
    """
    if latent_source == "sae":
        return compute_hidden_activations(sae, activations)
    if latent_source == "raw":
        return activations.reshape(activations.size(0), -1)
    raise ValueError(f"latent_source must be one of {LATENT_SOURCES}, got {latent_source!r}")
...
def extract_batch_targets(
    ...
    sae_hidden = latent_features(sae, capture.activations, latent_source)
...
def infer_residual_dims(
    model: LitModel, sae: SparseAutoencoder, latent_source: str = "sae"
) -> tuple[int, int]:
    feature_dim = sae.encoder.weight.shape[0] if latent_source == "sae" else sae.input_dim
    return feature_dim, model.model.horizon * 2
...
class LitSAEScorerResidual(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if latent_source not in LATENT_SOURCES:
            raise ValueError(f"latent_source must be one of {LATENT_SOURCES}, got {latent_source!r}")
        ...
        self.sae_dim, self.traj_dim = infer_residual_dims(backbone_model, sae, latent_source)
        ...
        self.save_hyperparameters({
            ...
            if key not in {"self", "backbone_model", "sae", "capture"}
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default="sae",
        choices=LATENT_SOURCES,
        help="'sae' uses sparse latents; 'raw' is the control arm using the dense "
             "block activation the SAE reconstructs.",
    )
