...
if should_index: # Takes approx. 9 mins to index
    ...
    DATA_CATS = ["train", "test", "val"]

    start = time.time()
    for category in DATA_CATS:
        indexes = []

        for i, fn in enumerate(tqdm([file for file in os.listdir(DATA_DIR) if '.tfrecord' in file and file.startswith(category)])):
        ...
        with open(f'index_{category}.pkl', 'wb') as f:
